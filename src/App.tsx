...
export default function App() {
  ...
  const [file, setFile] = useState<File>({
    contents: Filesystem["readme"].file.contents as string,
    path: "readme",
    type: "text",
  });
  ...
  const editorFileChange = useCallback(
  ...
  const textEditorMemo = useMemo(
    () => (
      <Editor
        ...
        value={file.contents}
        language={file.type}
        onChange={editorFileChange}
        onMount={editorDidMount}
      />
    ),
    [file, editorFileChange]
  );
  ...
  async function handleFileChange(reqfile: string) {
    ...
    switch (reqfile.split(".").pop()) {
      case "js":
        void setFile({
          contents: await readFileFromContainer(
            webcontainerInstance.current!,
            reqfile
          ),
          path: reqfile,
      ...
      default:
        void setFile({
          ...
          path: reqfile,
          type: reqfile.split(".").pop() as string,
        });
        break;
    }
  }
  ...
  return (
    <>
      <div>
        <div className="justify-between flex bg-background drop-shadow-md">
          <div className="flex justify-between">
            {Object.keys(filesystem).map((_file) => (
              <button
                key={_file}
                value={_file}
                className={`w-fit p-2 text-text font-normal ${
                  file.path === _file
                    ? "bg-tabActive"
                    : "bg-tab border-l border-tabLine"
                }`}
                onClick={() => {
                  handleFileChange(_file);
                }}
              >
                {_file}
              </button>
          ...
          <div className="px-2 justify-between">
            <button
              value="browser"
              className="w-fit p-2 bg-tab text-text font-normal"
              onClick={() => {
                setIsBrowserOpen(!isBrowserOpen);
              }}
            >
              {isBrowserOpen ? "Close" : "Open"} Browser
            </button>
            <button
              value="download"
              className="w-fit p-2 bg-tab text-text font-normal"
              onClick={async () => {
                await exportWebcontainer(webcontainerInstance.current!);
              }}
            >
              Download Devbox
            </button>
            <button
              ...
              className="w-fit p-2 bg-tab text-text font-normal"
              onClick={async () => {
                await importWebcontainer(
                  webcontainerInstance.current!,
                  setFilesystem as (filesystem: FileTree) => void
                );
      ...
      <pre
        id="console"
        style={{ height: "30vh", background: "#1e1e1e" }}
        className="bg-black w-full overflow-hidden border border-line"
        ref={terminalDom}
      ></pre>
    </>
  );
